...
public class RealizarRecebimentoService(RecebimentoApi recebimentoApi)
{
    public async Task<Result<Recebimento>> CapturarAsync(CapturaTransacaoContext capturaTransacaoContext, CancellationToken cancellationToken)
    {
        ...
        var transacaoCapturada = await recebimentoApi.CapturarAsync(capturaTransacaoContext, cancellationToken).ConfigureAwait(false);
